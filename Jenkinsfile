...
sudo docker run --name=db11.2.0.2-xe --shm-size=1g --restart=unless-stopped -e ORACLE_PWD=oracle localhost:5000/oracle/database:11.2.0.2-xe &'''
            
          },
          "Wait for successful start": {
            sh '''echo wait for successful start
sudo docker exec db11.2.0.2-xe bash -c 'yum -y install wget'
sudo docker exec db11.2.0.2-xe bash -c ' wget http://localhost:8080/apex --retry-connrefused --tries=0 -q --wait=3 --spider --connect-timeout=60'
echo show container
sudo docker ps'''
